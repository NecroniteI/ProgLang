...
Ключевые слова имеют специальное значение в синтаксисе языка, поэтому нелья их использовать в качестве имен переменных.

Например, в C++ запись

int return = 5;  
будет ошибочной потому что return является ключевым словом.
